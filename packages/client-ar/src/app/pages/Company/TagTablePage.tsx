...
const TagTable = ({actors}: any) => {
    ...
    const {
    ...
    }: any = useTable<{ pageIndex: number, pageSize: number }>(
        {
            ...
            defaultColumn,
            filterTypes,
            initialState: {pageSize: 10, sortBy: [{id: 'firstName', desc: false}]} as any,
        },
        useGlobalFilter,
        useFilters,
        useSortBy,
            ...
            <div>
                <TextField
                    ...
                    onChange={async (e) => {
                        ...
                        if (newValue === '') {
                            setFilter(undefined);
                            return setGlobalFilter(undefined)
                        }
                        setFilter(newValue);
                        setGlobalFilter(newValue) // Set undefined to remove the filter entirely
                    }}
                    ...
                    {headerGroups.map((headerGroup: any) => (
                        <TableRow {...headerGroup.getHeaderGroupProps()} hover>
                            {headerGroup.headers.map((column: any) => (
                                <TableCell {...column.getHeaderProps(column.getSortByToggleProps())} style={{ width: 100 }}>
                                    <div>
                                        {column.render('Header')}
                                        <span>
                    {column.isSorted ? (column.isSortedDesc ? <ArrowDownwardIcon color={'secondary'}/> : <ArrowUpwardIcon color={'secondary'}/>) : ''}
                  </span>
                                    </div>
...
const TagTablePage = (props: any) => {
    ...
    const actors = data.getTagsForTagsPage.filter((tag: any) => tag.tag === currentTag).map((tag: any) => tag.for);
    return (
        <Container>
